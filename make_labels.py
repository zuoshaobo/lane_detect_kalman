...
def lane_detection(img):
	...
	nonzerox = np.array(nonzero[1]) # Current positions to be updated for each window leftx_current = leftx_base rightx_current = rightx_base # Set the width of the windows +/- margin
        margin = 100
        ...
        for window in range(nwindows):
            ...
            if left_tracker == True and right_tracker == True:
                good_left_inds, leftx_current = left_line_detect(out_img, leftx_base, margin, minpix, nonzerox, nonzeroy, win_y_low, win_y_high, window_max, counter)
                good_right_inds, rightx_current = right_line_detect(out_img, rightx_base, margin, minpix, nonzerox, nonzeroy, win_y_low, win_y_high, window_max, counter)
